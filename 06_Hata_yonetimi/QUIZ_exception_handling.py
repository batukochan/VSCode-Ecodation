...
def listeMi(l1):
    ...
    assert type(l1) == list, AssertionError("liste tipinde veri giriniz...")
...
# print(listeToplami([1, 'b', 3]))
# print(listeToplami([1, 2, 3]))
...
# print(listeToplamiElse([1, 'b', 3]))
# print(listeToplamiElse([1, 2, 3]))
...
def listeSayilarininToplami(liste):
    ...
    try:
        ...
        for sayi in liste:
            if type(sayi) == int:
                ...
                sayiOlmayanlar.append(sayi)
    except Exception as a:
        raise a

    else:
        print(f'''listedeki sayıların toplamı = {toplam}
listede sayı olmayan elemanlar = {sayiOlmayanlar}''')


# listeSayilarininToplami([1, 'a', 'b', 3])
...
def listedeki_sayilar_toplami(liste):
    ...
    for i in liste:
        try:
            ...
            print(f'{i} elemanı sayı değildir.')
        else:
            toplam += i
    return toplam
...
def hesapMakinesi():
    ...
    if len(elemanlar) != 3:
        raise Exception('Ara boşluklu 3 eleman gir...')

    islemler = ('+', '-', '*', '/')
    sembol = elemanlar[1]
    if not sembol in islemler:
        raise Exception(f"{sembol}, 4 işlemden biri değil...")

    try:
        sayi1 = float(elemanlar[0])
        sayi2 = float(elemanlar[2])
    except:
        raise Exception('Elemanlar sayısal olmalı...')
    else:
        # sorun yoksa işlem yapılır
        if sembol == '+':
            sonuc = sayi1 + sayi2
        elif sembol == '-':
            sonuc = sayi1 - sayi2
            ...
            sonuc = sayi1 / sayi2
            assert sayi2 != 0, 'Bölen 0 olamaz'
            sonuc = sayi1 / sayi2
        return f"{sayi1} {sembol} {sayi2} = {sonuc}"
